...
public class BoardTest {
    ...
    public void testBoardInitialization() {
        for (int i = 0; i < Board.SIZE; i++) {
            for (int j = 0; j < Board.SIZE; j++) {
                assertEquals("Board should be initialized with 0 at cell (" + i + "," + j + ").", 0, board.getCell(i, j));
            }
        }
    }
    ...
    public void testSetCell() {
        board.setCell(0, 0, 1);
        assertEquals("Cell (0,0) should be set to 1.", 1, board.getCell(0, 0));
    }
    ...
    public void testReset() {
        board.setCell(0, 0, 1);
        board.reset();
        assertEquals("Board should be cleared after reset.", 0, board.getCell(0, 0));
    }
